...
from tensorforce import Runner, util, Environment
...
class TensorforceWorker(Worker):
    ...
    def compute(self, config_id, config, budget, working_directory):
        ...
        agent = dict(
            policy=policy, memory={'capacity': 1000, 'min_capacity': 1, 'type': 'replay'},
            update=update, optimizer=optimizer, objective=objective,
            reward_estimation=reward_estimation, baseline=baseline_policy,
...
def main():
    ...
    if args.environment is None:
        args.environment = Environment.create(environment=Simulated('training_data.csv'))
